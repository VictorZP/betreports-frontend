...
const FiltersSidebar: React.FC<FiltersSidebarProps> = ({ isOpen, onClose, onApplyFilters }) => {
  ...
  const handleApply = () => {
    const filters = {
      ...
      tournaments: selectedTournaments.length === tournaments.length ? null : selectedTournaments.join(','),
  ...
  const inputStyles = "w-full mt-1 px-3 py-2 bg-gray-700/40 text-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/50 transition-all border border-gray-600/30";
  const selectStyles = "w-full px-3 py-2 bg-gray-700/40 text-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/50 transition-all border border-gray-600/30 appearance-none cursor-pointer";
